...
async function checkRole(browser, email, password, roleName) {
    ...
    const url = page.url();
    console.log(`[${roleName}] Navigated to ${url}`);

    // Desktop Screenshot
    const deskShot = path.join(artifactDir, `dashboard_${roleName}_desktop.png`);
    await page.screenshot({ path: deskShot, fullPage: true });

    // Switch to Mobile
    await page.setViewport({ width: 375, height: 812, isMobile: true, hasTouch: true });
    await sleep(800);
    const mobShot = path.join(artifactDir, `dashboard_${roleName}_mobile.png`);
    ...
    const audit = await page.evaluate(() => {
        ...
        const tables = Array.from(document.querySelectorAll('table')).map(t => {
            ...
            return {
                ...
                hasScrollParent: parentComp.overflowX === 'auto' || parentComp.overflowX === 'scroll'
            };
        });
    ...
    console.log(`[${roleName}] Audit:`, JSON.stringify(audit));
    await context.close();
}
...
async function main() {
    ...
    try {
        await checkRole(browser, 'mentor.kominfo@surabaya.go.id', 'password', 'mentor');
        await checkRole(browser, 'dosen.unesa@unesa.ac.id', 'password', 'dosen');
        await checkRole(browser, 'admin@unesa.ac.id', 'password', 'universitas');
    } finally {
        await browser.close();
    }
}
