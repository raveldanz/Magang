<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <!-- Executive Macro Stats se-Kota Surabaya (Clean Minimalist Typography) -->
            @if(isset($macroStats))
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs hover:shadow-xs transition">
                        ...
                        <div class="mt-2 text-3xl font-black text-slate-900 tracking-tight">{{ $macroStats['total_agencies'] }}</div>
                    ...
                    <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs hover:shadow-xs transition">
                        ...
                        <div class="mt-2 text-3xl font-black text-slate-900 tracking-tight">{{ $macroStats['total_units'] }}</div>
                    ...
                    <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs hover:shadow-xs transition">
                        ...
                        <div class="mt-2 text-3xl font-black text-slate-900 tracking-tight">{{ $macroStats['total_quota'] }}</div>
                    ...
                    <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs hover:shadow-xs transition">
                        ...
                        <div class="mt-2 text-3xl font-black text-slate-900 tracking-tight">{{ $macroStats['total_staff'] }}</div>
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 shadow-xs hover:shadow-md transition p-6 flex flex-col h-full">
                        ...
                        <div class="flex-1 flex flex-col">
                            ...
                            <div class="flex items-start justify-between gap-4 shrink-0">
                                ...
                                @endphp
                                <div class="w-14 h-14 rounded-2xl bg-white border border-slate-200 shadow-2xs flex items-center justify-center p-2.5 shrink-0 overflow-hidden" style="width: 56px; height: 56px; min-width: 56px; min-height: 56px; max-width: 56px; max-height: 56px;">
                            ...
                            <div>
                                @if(($agency->total_admins ?? 0) === 0)
                                    <form method="POST" action="{{ route('admin.agencies.create_account', $agency->id) }}" class="inline-block m-0">
                                        @csrf
                                        <button type="submit" class="px-3.5 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-semibold transition shadow-xs cursor-pointer active:scale-95" title="Buatkan Akun Admin Dinas">
                                            Buat Akun
                                        </button>
                                    </form>
                                @elseif($isSuperAdmin && $firstAdmin)
                                    <form action="{{ route('admin.impersonate', $firstAdmin->id) }}" method="POST" class="inline-block m-0">
                                        @csrf
                                        <button type="submit" class="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200/80 rounded-xl text-xs font-semibold transition cursor-pointer active:scale-95" title="Masuk Sebagai Admin Dinas (Login As)">
                                            Login As
                                        </button>
                                    </form>
                                @endif
