<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {{-- 1. HERO BANNER GOVERNANCE HUB (PALING ATAS) --}}
        <div class="bg-gradient-to-r from-blue-900 via-blue-800 to-indigo-900 rounded-3xl p-6 sm:p-8 text-white shadow-xl shadow-blue-950/20 relative overflow-hidden">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 relative z-10">
                <div class="space-y-2 max-w-2xl">
                    ...
                    <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight">Pusat Kendali & Tata Kelola Eksekutif</h1>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 shrink-0">
                    <a href="{{ route('admin.agencies.index') }}" class="p-3.5 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-2xl border border-white/10 text-center transition group">
                        <div class="text-xl mb-1 group-hover:scale-110 transition-transform">🏢</div>
                        <div class="text-xs font-bold text-white">Instansi</div>
                        <div class="text-[10px] text-blue-200">Kelola Kuota</div>
                    </a>
                    <a href="{{ route('admin.universities.index') }}" class="p-3.5 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-2xl border border-white/10 text-center transition group">
                        <div class="text-xl mb-1 group-hover:scale-110 transition-transform">🎓</div>
                        <div class="text-xs font-bold text-white">Kampus</div>
                        <div class="text-[10px] text-blue-200">Mitra MBKM</div>
                    </a>
                    <a href="{{ route('admin.users.index') }}" class="p-3.5 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-2xl border border-white/10 text-center transition group">
                        <div class="text-xl mb-1 group-hover:scale-110 transition-transform">👥</div>
                        <div class="text-xs font-bold text-white">Pengguna</div>
                        <div class="text-[10px] text-blue-200">Semua Akun</div>
                    </a>
                    <a href="{{ route('admin.audit_logs.index') }}" class="p-3.5 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-2xl border border-white/10 text-center transition group">
                        <div class="text-xl mb-1 group-hover:scale-110 transition-transform">📜</div>
                        <div class="text-xs font-bold text-white">Log Audit</div>
                        <div class="text-[10px] text-blue-200">Rekam Jejak</div>
                    </a>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {{-- Pendaftar --}}
            <div class="bg-white p-4 sm:p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition">
        ...
        {{-- 3. DISTRIBUSI SEBARAN KAMPUS & INSTANSI DENGAN TRACK SOLID --}}
                    ...
                    @forelse($agenciesList as $agency)
                        ...
                        <div>
                            <div class="flex items-center justify-between text-xs font-semibold mb-1.5">
                                ...
                                <span class="text-blue-700">{{ $agCount }} Mahasiswa <span class="text-slate-400 font-normal">({{ $agPercentage }}%)</span></span>
                            </div>
                            <div class="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden">
                                <div class="bg-blue-600 h-2.5 rounded-full transition-all duration-500" style="width: {{ max((float)$agPercentage, 2) }}%"></div>
                            </div>
                        </div>
                    @empty
                    ...
                    @forelse($campusList as $campus)
                        ...
                        <div>
                            <div class="flex items-center justify-between text-xs font-semibold mb-1.5">
                                ...
                                <span class="text-sky-700">{{ $campCount }} Mahasiswa <span class="text-slate-400 font-normal">({{ $campPercentage }}%)</span></span>
                            </div>
                            <div class="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden">
                                <div class="bg-sky-500 h-2.5 rounded-full transition-all duration-500" style="width: {{ max((float)$campPercentage, 2) }}%"></div>
                            </div>
                        </div>
                    @empty
