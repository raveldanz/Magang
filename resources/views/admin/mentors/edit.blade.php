<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.mentors.index') }}" class="p-2 rounded-xl bg-white border border-gray-200 text-gray-600 hover:text-gray-900 transition flex items-center justify-center shadow-xs" title="Kembali">
            ...
            <div>
                <h2 class="font-black text-xl text-gray-900 tracking-tight">
                    Edit Data Mentor Lapangan
                </h2>
                <p class="text-xs text-gray-500">Perbarui identitas, instansi, atau status keaktifan mentor</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl border border-gray-100 shadow-xs p-6 sm:p-8">
                ...
                <form method="POST" action="{{ route('admin.mentors.update', $mentor->id) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $mentor->name) }}" required class="w-full text-xs sm:text-sm border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 shadow-2xs">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email', $mentor->email) }}" required class="w-full text-xs sm:text-sm border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 shadow-2xs font-mono">
                    </div>
                        ...
                        <div>
                            ...
                            <select name="agency_profile_id" required class="w-full text-xs sm:text-sm border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 shadow-2xs">
                                @foreach($agencies as $ag)
                                    <option value="{{ $ag->id }}" {{ old('agency_profile_id', $mentor->agency_profile_id) == $ag->id ? 'selected' : '' }}>{{ $ag->agency_name }}</option>
                                @endforeach
                            </select>
                        </div>
                    @endif
