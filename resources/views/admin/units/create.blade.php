<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ $returnTo ?? request('return_to', route('admin.units.index')) }}"
                class="p-2 bg-white hover:bg-gray-100 border border-gray-200 text-gray-700 rounded-xl transition shadow-sm">
            ...
            <div>
                <h2 class="font-bold text-2xl text-gray-800 leading-tight">
                    Tambah Divisi / Lowongan Magang Baru
                </h2>
                <p class="text-xs text-gray-500 mt-0.5">Buka divisi kerja baru dan tentukan kuota penerimaan mahasiswa
                </p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white rounded-2xl p-6 sm:p-8 border border-gray-200 shadow-sm">
                ...
                <form action="{{ route('admin.units.store') }}" method="POST" class="space-y-6">
                    ...
                    @else
                        <input type="hidden" name="agency_profile_id" value="{{ $defaultAgencyId }}">
                    @endif
                    ...
                    <div>
                        ...
                        <input type="number" id="quota" name="quota" min="0" max="100" value="{{ old('quota', 5) }}"
                            class="mt-1 block w-full border-gray-300 rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm"
                            required>
                        <p class="text-xs text-gray-500 mt-1">Jumlah maksimal mahasiswa yang dapat diterima di divisi
                            ini.</p>
                        @error('quota')
                            <span class="text-rose-600 text-xs font-medium block mt-1">{{ $message }}</span>
                        @enderror
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4"
                            ...
                            class="mt-1 block w-full border-gray-300 rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">{{ old('description') }}</textarea>
                        @error('description')
                            <span class="text-rose-600 text-xs font-medium block mt-1">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="pt-4 border-t border-gray-100 flex justify-end gap-3">
                        <a href="{{ $returnTo ?? request('return_to', route('admin.units.index')) }}" class="...">
                            Batal
                        </a>
                        <button type="submit"
                            class="px-7 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-xl shadow-md transition cursor-pointer">
