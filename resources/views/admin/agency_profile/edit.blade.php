            ...
            <form action="{{ route('admin.agency_profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="government_name" name="government_name" type="text" class="mt-1 block w-full" 
                                value="{{ old('government_name', $profile->government_name) }}" required />
                        </div>
                    ...
                    <div class="mt-4">
                        ...
                        <textarea id="address" name="address" rows="2" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm">{{ old('address', $profile->address) }}</textarea>
                    </div>
                    ...
                    <div class="mt-4 pt-4 border-t">
                        ...
                        <div class="flex items-center space-x-4 mt-2">
                            @if ($profile->logo)
                                <img src="{{ asset('storage/' . $profile->logo) }}" alt="Logo Instansi" class="w-16 h-16 object-contain border p-1 rounded-lg bg-gray-50">
                            @else
                                <div class="w-16 h-16 bg-gray-100 border border-dashed rounded-lg flex items-center justify-center text-xs text-gray-400">Tanpa Logo</div>
                            @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <x-text-input id="signee_name" name="signee_name" type="text" class="mt-1 block w-full" 
                                value="{{ old('signee_name', $profile->signee_name) }}" required />
                        </div>
                        ...
                        <div>
                            ...
                            <x-text-input id="signee_nip" name="signee_nip" type="text" class="mt-1 block w-full" 
                                value="{{ old('signee_nip', $profile->signee_nip) }}" />
                        </div>
                        ...
                        <div>
                            ...
                            <x-text-input id="signee_position" name="signee_position" type="text" class="mt-1 block w-full" 
                                value="{{ old('signee_position', $profile->signee_position) }}" required />
                        </div>
                    </div>
